...
class ECSService(object):
    ...
    def register_task_definition(self, family, file, volumes, execution_role_arn):
        """
        ...
        :param execution_role_arn: the task execution role
        :return: the response or raise an Exception
        """
        ...
        if os.path.isfile(volumes) is False:
            response = self.client.register_task_definition(family=family, containerDefinitions=container_definitions, executionRoleArn=execution_role_arn)
        else:
            with open(volumes, 'r') as content_volumes:
                container_definitions_volumes = json.loads(content_volumes.read())
            response = self.client.register_task_definition(family=family, containerDefinitions=container_definitions, volumes=container_definitions_volumes, executionRoleArn=execution_role_arn)
